...
{
    //    private static final int PARTITION_COUNT = 4; // TODO FIXME
            ...
            throws Exception
    {
        ...
        try (OperatorFactory joinOperatorFactory = innerJoinOperatorFactory(lookupSourceFactory, probePages);
            ...
            ListenableFuture<LookupSourceProvider> lookupSourceBuilt = buildSideSetup.lookupSourceFactory.createLookupSourceProvider();
            ...
            while (!lookupSourceBuilt.isDone()) {
                for (int i = 0; i < buildOperatorCount; i++) {
                    ...
                    if (whenSpill.get(i) == WhenSpill.DURING_BUILD && buildOperator.getOperatorContext().getReservedRevocableBytes() > 0) {
                        checkState(!lookupSourceBuilt.isDone(), "Too late, LookupSource already done");
    ...
    private void buildLookupSource(BuildSideSetup buildSideSetup)
    {
        ...
        Future<LookupSourceProvider> lookupSourceBuilt = buildSideSetup.lookupSourceFactory.createLookupSourceProvider();
        List<Driver> buildDrivers = buildSideSetup.buildDrivers;
            ...
            implements SingleStreamSpillerFactory
    {
        ...
        public SingleStreamSpiller create(List<Type> types, SpillContext spillContext, LocalMemoryContext memoryContext)
        {
            return new SingleStreamSpiller()
            {
                ...
                {
                    writing = false;
                    return immediateFuture(spills.build());
                }
