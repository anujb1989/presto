        ...
        implements Operator, Closeable
{
    ...
    private Page spillAndMaskSpilledPositions(Page page, IntPredicate spillMask)
    {
        ...
        if (!spiller.isPresent()) {
            spiller = Optional.of(partitioningSpillerFactory.create(
                    probeTypes,
                    new LocalPartitionGenerator(hashGenerator, lookupSourceFactory.partitions()),
    ...
    private boolean tryUnspillNext()
    {
        if (unspilledInputPages.hasNext()) {
            ...
            unspilledInputPages = spiller.map(spiller -> spiller.getSpilledPages(currentPartition.get().number()))
    ...
    private void processProbe()
    {
        try (LookupSourceLease lookupSourceLease = lookupSourceProvider.leaseLookupSource()) {
            ...
            // TODO handle the case whe lookupSourceLease's epoch is different

            // TODO reuse
            LocalPartitionGenerator partitionGenerator = new LocalPartitionGenerator(hashGenerator, lookupSourceFactory.partitions());

            int currentRowPartition = partitionGenerator.getPartition(probe.getPosition(), probe.getPage());

            probe = null;
            // TODO Wrong Thing To Do™
        }
    }
